...
public class SonataMain {
    public static void main(String[] args) {
        ...
        System.out.println(himCar2);
        System.out.println(himCar==himCar2);//false
    }
}
/*
DI에 대한 실습 예제
new로 직접 생성하는 객체와 스프링 컨테이너가 만들어주는 객체 차이를
눈으로 확인해보자.
- new로 만든 객체는 만든 사람이 직접 생명주기를 관리한다.(위임이 아니다)
- 스프링 컨테이너에서 꺼낸 Bean은 스프링이 대신 생성/관리한다.
- scope='singleton'이면 항상 같은 객체,
scope='prototype'이면 요청할 때마다 새로운 객체가 나옴

1. myCar
: class - Sonata
: scope - singleton
- 스프링 컨테이너 안에서 딱 1개만 생성
- 이후 getBean("myCar") 몇번 호출해도 같은 객체 주소번지

2. himCar
: class - Sonata
: scope - prototype
- getBean("himCar") 호출할 때마다 새로운 객체 주소번지
